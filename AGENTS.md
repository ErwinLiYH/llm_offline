...
- Override eval checkpoint/model path from the CLI: `micromamba run -n llm_offline python evaluate.py --config eval.yaml --model_path <checkpoint_or_model_id>`. The path may contain one trailing `*` wildcard, for example `--model_path '.../ep7*'`; it must match exactly one path.
...
- The Slurm eval wrapper also accepts layered configs and model-path override: `sbatch sbatch/evaluate.isb.slurm --config base.yaml override.yaml --model_path <checkpoint_or_model_id>`. Quote trailing wildcard paths in shell commands so `evaluate.py` can enforce the single-match rule.
...
Data flow:
- dataset
- episode-level train/val split using randomized sampling: first draw up to `episode_keep_num` episodes as a sampled pool (if the dataset has fewer episodes, use all), optionally overridden per selected variant by `episode_keep_per_varient`, then split inside that pool with `floor(pool_size * train_data_ratio)` train episodes and the remainder as val episodes
...
- Local PointMaze offline data is generated by `local_pointmaze_gen.py`. Its default behavior truncates each generated episode at first success; `--post-success-hold-steps > 0` records additional fixed-goal hold transitions using a PD hold controller. Enabling hold data for an existing local dataset should use `--overwrite` to avoid mixing old goal-arrival-only episodes with hold episodes.
- Training and `estimate_dataset.py` can override PointMaze/AntMaze local dataset lookup with `local_dataset_root`; omit it or set it to `null` to use the variant default `local_datasets/<family>-<variant>-v0`. The value may be a parent directory containing multiple local datasets or a direct path to one variant dataset directory. `local_dataset_path` is accepted as a compatibility alias, but setting both aliases to different values is an error.
- `pointmaze_data_config` is PointMaze-only and supports `truncate` plus `truncate_holding`, both applied after raw episode loading and before `episode_keep_num` / `episode_keep_per_varient` sampling and train-val splitting. `truncate: true` cuts at the first `infos.success` transition; `truncate_holding: N` keeps `N` extra action samples after that transition. PointMaze does not support AntMaze's `filter_success` or fall detection.
...
- PointMaze and AntMaze `format_obs` emit dynamic `location_sensing_en` / `location_sensing_zh` and `wall_sensing_en` / `wall_sensing_zh`. Location sensing describes the current cell and goal cell using 1-based row/column indexing from the top-left corner. Shared coordinate-to-cell conversion applies the floor/map-center formula with the family map scaling; if that raw cell is a wall, it snaps to the nearest free cell center so prompts do not report wall cells as positions. Wall sensing is versioned by `wall_sensing_version` (`v1`-`v5`) and `map_sensing_boundary_risk_threshold`, which defaults/nulls resolve to `v3` and `0.10`. The threshold is a fraction of cell size. Default `v3` reports a free movement neighbor as `wall` only for new-corner geometry: near the side boundary, current same-side cell free, and forward same-side diagonal cell wall. `v5` is the newer risk semantics where direct walls stay `wall` and the same new-corner geometry reports `risk`.
- Standalone eval and `score.py mode: score` inherit `wall_sensing_version` and `map_sensing_boundary_risk_threshold` from checkpoint `config.yaml` when present. If a checkpoint records these fields, eval/score YAML may only repeat matching values; conflicting values are rejected. Older checkpoints without these fields use eval/score YAML values or the default `v3` / `0.10`.
...
- Eval and score rollout use process-isolated env workers through `utils/rollout/`. `rollout_worker_num` is per eval/score rank; in DDP eval the maximum env worker count is roughly `world_size * rollout_worker_num` for ranks with assigned variants. Each worker subprocess owns one env, while the parent rank owns the model and serves action requests.
- `rollout_worker_lifetime: slot` keeps one long-lived worker per slot and runs multiple episodes sequentially; `episode` starts a fresh worker subprocess per episode. Worker crashes are retried according to `rollout_worker_retries` and recorded in `worker_failures` instead of killing the parent process.
- Worker subprocesses do not normally print per-step or per-episode progress. The command line shows parent-process startup, variant summaries, and result paths; detailed step logs/videos are written under episode artifact directories and worker failures are recorded in result JSON.
...
- Multi-variant training, including `all` and `except`, uses weighted sampling by variant sample count. Optional `balance_variant_episode_count: true` first equalizes the sampled episode pool size across selected variants to the smallest per-variant target. If `episode_keep_per_varient` is configured, its per-variant keep values take precedence and balancing is skipped with a warning.
...
- Training config `model_path` initializes a fresh training run from an existing checkpoint's LoRA adapter/tokenizer and continuous/MTP sidecar decoder without restoring optimizer, LR scheduler, or loop state. Use this for second-stage or transfer fine-tuning. `resume_from_checkpoint` is strict interrupted-run resume: it also restores `trainer_state.pt`, optimizer, scheduler, and loop position, and is incompatible with changing the data mix or partition plan.
- `estimate_dataset.py` is CPU/tokenizer-only and does not build official dataset caches. It fully loads raw trajectories, applies the same training variant selection, local dataset root override, prompt selection, `episode_keep_num` plus optional `episode_keep_per_varient`, `train_data_ratio`, `balance_variant_episode_count`, action-mode, and family data preprocessing (`antmaze_data_config` for AntMaze, `pointmaze_data_config` for PointMaze), then tokenizes a small number of complete sampled episodes per selected variant. Tokenized `.pkl` size and tokenized `dataset._samples` Python object memory are both extrapolated by selected step ratio, not by epoch sample count. The memory estimate uses recursive `sys.getsizeof` over tokenized sample Python objects and excludes raw episodes, tokenizer/model/optimizer, DataLoader worker copies, prefetch batches, gradients, and activations. With `dataset_load_partitions > 1`, the report includes the peak resident train shard estimate plus val split. `--world_size` is only a mathematical DDP batch-count estimate and does not initialize distributed training or require real ranks/GPUs.
- `config.yaml` controls the base model via `model_name`, optional fresh checkpoint initialization via `model_path`, whether Unsloth uses 4-bit loading via `load_in_4bit`, LoRA module and optional decoder-layer filtering via `lora_target_modules` / `lora_layers_to_transform`, parallel training via `parallel_backend` / `ddp_find_unused_parameters` / `distributed_timeout_seconds`, optional latest-only system monitoring via `resource_monitor_enabled` / `resource_monitor_interval_seconds`, which prompt templates are used for dataset construction via `prompt_templete_index`, tokenization worker count via `dataset_workers` (per rank), wall sensing via `wall_sensing_version` / `map_sensing_boundary_risk_threshold`, local dataset lookup via `local_dataset_root`, tokenized dataset sharding via `dataset_load_partitions`, train/val batch loading via `dataloader_config` (`num_workers`, `pin_memory`, `persistent_workers`, `prefetch_factor`, `non_blocking`), action encoding via `action_token_mode` / `action_num_bins` / `mtp_k` / `mtp_lcm_weight` / `mtp_quadratic_decoding` / `new_token` / `action_bin_min` / `action_bin_max` / `action_soft_label_sigma` / `action_soft_label_radius` / `gaussian_log_std_init` / `gaussian_log_std_min` / `gaussian_log_std_max` / `student_t_df` / `continuous_mean_l1_weight` / `action_head_dropout` / `action_head_weight_decay`, rollout worker parallelism via `rollout_worker_num` / `rollout_worker_lifetime` / `rollout_worker_retries` / `rollout_worker_start_timeout_seconds` / `rollout_action_timeout_seconds` / `policy_batch_timeout_ms` / `eval_distribute_variants`, rollout action sampling via `action_sampling` / `action_temperature` / `action_top_p` / `action_top_k`, offline episode sampling via `episode_keep_num` / `episode_keep_per_varient` / `balance_variant_episode_count` / `sampling_seed`, history prompt settings via `history_num` / `history_stride`, training-time eval cadence and fixed episode seeds via `eval_step_interval` / `step_eval_skip` / `eval_seed`, eval step logging via `record_step_logs`, eval video recording via `record_video` / `record_all` / `video_episode_index` / `video_fps` / `video_format` / `video_save_workers` / `video_save_max_pending` / `mujoco_gl`, and the eval result root via `result_root`. `eval_parallel_episodes` is deprecated and rejected; use `rollout_worker_num`. `simple_mtp_bin` uses `mtp_lcm_weight` but not `mtp_k` or `mtp_quadratic_decoding`. `action_dim` is normally auto-resolved for parallel/continuous/MTP modes and saved into checkpoints rather than manually authored in `config.yaml`.
...
- In DDP, checkpoint saving and validation remain rank0-only. With `eval_distribute_variants: true`, training-time rollout variants are assigned round-robin across ranks; the owning rank writes that variant's result, step logs, and videos, while rank0 gathers summaries and writes W&B metrics. Training-time rollout is always isolated: each rank launches a single-process `evaluate.py` subprocess for its assigned variants using the just-saved checkpoint, with DDP environment variables removed and `CUDA_VISIBLE_DEVICES` narrowed to the parent rank's local GPU. Child `evaluate.py` failures warn, set W&B rollout failure metrics on rank0, and do not stop training. With `dataset_cache_dir`, rank0 builds tokenized caches before other ranks read them after a barrier.
...
- Checkpoints are stored under `checkpoints/<env_family>/<model_slug>/<selection_tag>/<experiment_id>/`.
  - `selection_tag` is the single variant name, `all`, `all-<N>v-<hash>` for configured `all` subsets, or `except-<N>x-<hash>` for `except` exclusions. Full readable tags are saved in config fields such as `train_selection_tag_full`.
- Epoch checkpoints use `ep<N>(step<M>)`, where `M` is the completed global train batch step at epoch end; step checkpoints use `step<N>`, and final checkpoints use `final`.
...
- Training-time eval results live under `<result_root>/<model_slug>/train=<env_family>-<selection_tag>/exp=<experiment_id>/ep<N>(step<M>)/eval=<env_family>-<variant>/result.json` for epoch evals or `.../step<N>/eval=<env_family>-<variant>/result.json` for step evals. The merged eval config is saved at the corresponding `ep<N>(step<M>)/eval_config.yaml` or `step<N>/eval_config.yaml`, and per-attempt child config/stdout/stderr files live under `isolated_eval/rank_<rank>/attempt_<n>.*`.
