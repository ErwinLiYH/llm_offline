...
- Training-time eval results live under `<result_root>/<model_slug>/train=<env_family>-<selection_tag>/exp=<experiment_id>/epoch_<n>/eval=<env_family>-<variant>/result.json` or `.../step<N>/eval=<env_family>-<variant>/result.json`. The merged eval config is saved at the corresponding `epoch_<n>/eval_config.yaml` or `step<N>/eval_config.yaml`, and per-attempt child config/stdout/stderr files live under `isolated_eval/rank_<rank>/attempt_<n>.*`.
