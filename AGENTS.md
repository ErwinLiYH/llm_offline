...
- `dataset_load_partitions > 1` enables low-memory tokenized-data training. Raw trajectories are still loaded normally, then each variant's selected train episodes are deterministically shuffled and split into fixed episode shards. The full val split is tokenized/loaded once and kept resident; only train shards are loaded/trained one at a time. `dataset_cache_dir` is required. One epoch still means all train shards are trained once, with shard order shuffled per epoch, and step eval keeps the normal global-batch/gradient-accumulation timing instead of waiting for shard boundaries.
