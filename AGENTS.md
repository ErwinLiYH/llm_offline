...
- `dataset_load_partitions > 1` enables low-memory tokenized-data training. Raw trajectories are still loaded normally, then each variant/split's selected episodes are deterministically shuffled and split into fixed episode shards. Each shard is tokenized into its own cache file and later loaded/trained one shard at a time; `dataset_cache_dir` is required. One epoch still means all shards are trained once, with shard order shuffled per epoch.
