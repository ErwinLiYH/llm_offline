...
- **Prompt templates:** training uses the first `prompt_template_count` templates from `prompts/<env_family>/<variant>.yaml`, and evaluation always uses template 0. The current PointMaze prompt files contain 5 templates (0–2 English, 3–4 Chinese), but the loader follows the actual number defined in each YAML file.
