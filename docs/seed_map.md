...
两者默认生成 `[0, 100)`、每个 map seed 50 条成功轨迹，并从最难的 400 个 reachable start/goal pair 中按 rank difficulty 加权采样（`HARD_SAMPLE_ALPHA=1.0`），与对应的非 seed-map hard 脚本一致。PointMaze/AntMaze 分别默认 `HARD_SAMPLE_MAX_PATH_LEN=40/25`：先在完整 reachable pair space 上计算并排序原 difficulty，跳过超过 family 限制的 pair，再继续向较低排名扫描直至补足 400 个 eligible pair；设为 `0` 可关闭该保险。PointMaze seed-map hard 与非 seed-map hard 同样默认记录 100 个 post-success hold steps；AntMaze 两种 hard 路径都保持 fixed-horizon、成功后不截断的语义。PointMaze 默认最终尺寸为 `9/11/13/15`，AntMaze 为 `9/11/13`。六个 DataGen Slurm 脚本都把 `DATASET_ROOT` 默认设为 HPC 环境变量 `${PROJECTDIR}/local_datasets`；脚本内部的 `PROJECT_ROOT` 仅用于仓库代码路径。seed-map resolver 会继续在该 base root 下加入 family/version/size/reward namespace，避免 PointMaze 和 AntMaze 的同名 `0-100-50` corpus 冲突。脚本默认续写已有 corpus；设置 `OVERWRITE=1` 才会重建。可通过 `SEED_MAP_START`、`SEED_MAP_END`、`TRAJECTORIES_PER_SEED`、`SEED_MAP_MIN_SIZE`、`SEED_MAP_MAX_SIZE`、`MAX_EPISODE_STEPS`、`POST_SUCCESS_HOLD_STEPS`、`POST_SUCCESS_HOLD_NOISE_STD`、`HARD_SAMPLE_TOP_N`、`HARD_SAMPLE_ALPHA`、`HARD_SAMPLE_MAX_PATH_LEN`、`NUM_WORKERS`、`SEED`、`REWARD_TYPE`、`DATASET_ROOT` 和 `TEMPORARY_DATASET_ROOT` 等环境变量覆盖。
