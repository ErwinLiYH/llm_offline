...
run_one() {
  ...
  local experiment_id="paperobs1-antmaze-${algorithm}-e300-2m-r100-s${SEED}"
  local run_dir="baseline_runs/${experiment_id}"

  if [[ -f "${run_dir}/summary.json" ]]; then
    printf '[paper obs v1 antmaze 2m] skip completed %s\n' "${experiment_id}"
    return 0
  fi
  if [[ -e "${run_dir}" ]]; then
    printf '[paper obs v1 antmaze 2m] refuse incomplete existing run %s\n' \
      "${experiment_id}" >&2
    return 1
  fi

  printf '[paper obs v1 antmaze 2m] start %s\n' "${experiment_id}"
  micromamba run -n llm_offline_baselines python baseline_train.py \
    ...
    "${CFG_DIR}/two_million.yaml" \
    --experiment_id "${experiment_id}"
  local status=$?
  printf '[paper obs v1 antmaze 2m] finish %s status=%d\n' \
    "${experiment_id}" "${status}"
  return "${status}"
}

# Keep at most two copies of the 4.5M-transition AntMaze training buffer in
# memory. This is the same safe scheduling used by the 500k and 1M runs.
pids=()
...
# MLP-BC is faster and starts after both actor-critic jobs release their buffers.
