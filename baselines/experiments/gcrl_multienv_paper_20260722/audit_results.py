...
OUTPUT = ROOT / "reports/gcrl_multienv_paper_20260722_checkpoint_audit.json"
...
def _run_id(algorithm: str, family: str) -> str:
    return f"gcrl-paper-{algorithm}-{family}-multienv-s0-1m-20260722"
...
def _audit_rollout(rollout: dict, expected_variants: list[str]) -> None:
    if set(rollout["variants"]) != set(expected_variants):
        raise AssertionError("rollout variant set does not match the configured train+test set")
    ...
    for family, groups in FAMILIES.items():
        ...
        for algorithm in ALGORITHMS:
            ...
            evaluations = {
                ...
                for record in summary["evaluation_history"]
            }
            rows = []
            for step in STEPS:
